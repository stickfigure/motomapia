...
public class WikiPlace implements Serializable, Comparable<WikiPlace>
{
	...
	public WikiPlace(JsonNode placeNode, boolean fromBox) throws WikimapiaException
	{
		if (fromBox)
		{
			...
			String idStr = placeNode.get("id").asText();
			this.id = Long.parseLong(idStr);

			String nameStr = placeNode.path("name").asText();
			if (nameStr != null)
		...
		{
			this.id = placeNode.get("id").asLong();

			String nameStr = placeNode.path("title").asText();
			if (nameStr != null)
				nameStr = nameStr.trim();
